...
#include "../include/social_robot_arm_libs/kinematics.h"
...
Eigen::MatrixXd SolverUsingCRAndJacobian::jacobian(Manipulator *manipulator, Name tool_name)
{
  ...
  for (int32_t size = 0; size < manipulator->getDOF(); size++)
  {
...
std::vector<JointValue> SolverUsingCRAndJacobian::inverseVelAccSolver(Manipulator manipulator, Name tool_name, Pose target_pose)
{
  ...
  for(int32_t index = 0; index < manipulator.getDOF(); index++)
  {
...
void SolverUsingCRAndJacobian::forwardSolverUsingChainRule(Manipulator *manipulator, Name component_name)
{
  ...
  Name parent_name = manipulator->getComponentParentName(my_name);
  int32_t number_of_child = manipulator->getComponentChildName(my_name).size();
  ...
  for (int32_t index = 0; index < number_of_child; index++)
  {
...
bool SolverUsingCRAndJacobian::inverseSolverUsingJacobian(Manipulator *manipulator, Name tool_name, Pose target_pose, std::vector<JointValue> *goal_joint_value)
{
  ...
  for (int32_t count = 0; count < iteration; count++)
  {
    ...
    //set changed angle
    int index = 0;
    Name my_name =  _manipulator.getWorldChildName();
    for (int32_t size = 0; size < _manipulator.getDOF(); size++)
    {
      if(_manipulator.checkComponentType(my_name, ACTIVE_JOINT_COMPONENT))
      {
        _manipulator.setJointPosition(my_name, _manipulator.getJointPosition(my_name)+delta_angle(index));
        index++;
      }
      my_name = _manipulator.getComponentChildName(my_name).at(0);
    }

//    //set changed angle
//    std::vector<double> changed_angle;
//    for(int32_t index = 0; index < _manipulator.getDOF(); index++)
...
bool SolverUsingCRAndSRJacobian::inverseSolverUsingSRJacobian(Manipulator *manipulator, Name tool_name, Pose target_pose, std::vector<JointValue> *goal_joint_value)
{
  ...
  //solver parameter
  double lambda = 0.0;
  const double param = 0.02;
  const int32_t iteration = 500;
...
//  std::vector<double> set_angle;                                                   //set angle (q + dq)
  ...
  Eigen::Vector3d target_orientation_rpy = math::convertRotationMatrixToRPYVector(target_pose.kinematic.orientation);
  ...
  log::print_vector(debug_target_pose,16);
  log::println("pre_pose");
  log::print_vector(debug_present_pose,16);
  log::println("delta_pose");
  log::print_vector(debug_target_pose-debug_present_pose,16);
  #endif
  ////////////////////////////debug//////////////////////////////////
  ...
  for (int32_t count = 0; count < iteration; count++)
  {
    ...
    for (int32_t index = 0; index < _manipulator.getDOF(); index++)
    {
      if(_manipulator.checkComponentType(my_name, ACTIVE_JOINT_COMPONENT))
      {
        _manipulator.setJointPosition(my_name, _manipulator.getJointPosition(my_name) + angle_changed(index));
      }
      my_name = _manipulator.getComponentChildName(my_name).at(0);
    }
...
//    _manipulator.setAllActiveJointPosition(set_angle);
    ...
    present_orientation_rpy = math::convertRotationMatrixToRPYVector(present_orientation);
    for(int t=0; t<3; t++)
      debug_present_pose(t) = present_position(t);
    for(int t=0; t<3; t++)
      debug_present_pose(t+3) = present_orientation_rpy(t);
    ...
    else
    {
      ...
      for (int32_t index = 0; index < _manipulator.getDOF(); index++)
      {
        if(_manipulator.checkComponentType(my_name, ACTIVE_JOINT_COMPONENT))
        {
          _manipulator.setJointPosition(my_name, _manipulator.getJointPosition(my_name)- (gamma * angle_changed(index)));
        }
        my_name = _manipulator.getComponentChildName(my_name).at(0);
      }
...
//    present_angle = _manipulator.getAllActiveJointPosition();
//    set_angle.clear();
//    for (int index = 0; index < _manipulator.getDOF(); index++)
