...
void social_recharge::laser_moving()
{
  ...
  while(ros::ok())
  {
    ...
    if((int) laser.size() > 5)
    {
      ...
      else if(target_position.x > 0.22 || fabs(target_position.z) > m_angle_threshold)	//충전기 위치가 400mm 이상 떨어졌을 대 정밀이동
      {
        wheel_command_laser_detail(target_position);
      }

      else if ((target_position.x < 0.21 && fabs(target_position.y) < 0.015))// || && m_charge == true)
      {
        ROS_INFO("SIGNAL IN !!!!\n");
        target_position.init();
        wheel_command_laser_detail(target_position);
      ...
      else if (target_position.x < 0.20)		//충전기 위치가 4000mm이하 각도도 문턱치 이하일 때 laser를 이용한 이동 종료
      {
        target_position.init();
        wheel_command_laser_detail(target_position);
        break;
...
void social_recharge::wheel_command_laser_detail(eun_u::Point3D position)
{
  ...
  if (m_charge == true)
  {
    ...
    else if(fabs(pose.y) > m_trans_threshold - 0.01)
    {
      if(pose.y > 0) msg.linear.y = -0.02;
      else msg.linear.y = 0.02;
    }
